...
function renderMathToHTMLClient(text) {
  if (!text) return ''
  // normalize smart quotes and common unicode characters that KaTeX doesn't like
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch('/api/count').then(r => r.json()).then(j => {
      setTotal(j.count || 0)
      // then ask progress endpoint for the next unanswered index
      fetch('/api/progress').then(r2 => r2.json()).then(p => {
        if (typeof p.nextIndex === 'number') setCurrent(Math.max(0, Math.min(j.count - 1, p.nextIndex)))
      }).catch(()=>{})
    }).catch(()=>{})
    // read optional workerId from URL params
  ...
  useEffect(() => {
    if (total>0) loadQuestion(current)
  }, [total, current])
